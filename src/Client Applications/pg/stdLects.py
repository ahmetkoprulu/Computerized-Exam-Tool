...
def on_lect_select(self, dropdown, txt):
    ...
    # self.CourseInfo = [Course for Course in self.DATA if Course["courseName"]==txt][0]
    ...
    #                                                  )
    # self.data_exams=["Ahmet","VEli","Ahmet","VEli","Ahmet","VEli","Ahmet","VEli","Ahmet","VEli","Ahmet","VEli","Ahmet","VEli"]
    # replace("_", " ").title()
    ...
    self.ids["list_exams"].adapter = ListAdapter(data=[i["name"] for i in self.CourseInfo["exams"]],
                                                 cls=ListItemButton,
...
def on_exam_select(self, dt):
    ...
    try:
        self.ids["txt_date_body"].text = str(datetime.datetime.strptime(find(self.CourseInfo["exams"],"name",self.ids["list_exams"].adapter.selection[0].text)["date"],
                                                                    "%Y-%m-%dT%H:%M:%S").strftime("%Y %m %d"))
        self.ids["txt_time_body"].text = str(datetime.datetime.strptime(find(self.CourseInfo["exams"],"name",self.ids["list_exams"].adapter.selection[0].text)["date"],
                                                                    "%Y-%m-%dT%H:%M:%S").strftime("%H:%M:%S"))
    except:
        self.ids["txt_date_body"].text = str(datetime.datetime.strptime(
            find(self.CourseInfo["exams"], "name", self.ids["txt_info_head"].adapter.selection[0].text)["date"],
            "%Y-%m-%dT%H:%M:%S").strftime("%Y %m %d"))
        self.ids["txt_time_body"].text = str(datetime.datetime.strptime(
            find(self.CourseInfo["exams"], "name", self.ids["txt_info_head"].adapter.selection[0].text)["date"],
            "%Y-%m-%dT%H:%M:%S").strftime("%H:%M:%S"))
...
def on_exam_join(self):
    ...
    print ""
    print  self.ids["txt_lect_code"].text
    print  self.ids["txt_lect_name"].text
    print self.ids["list_exams"].adapter.selection[0].text
