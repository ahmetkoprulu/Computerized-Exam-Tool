...
def on_pre_enter(self):
    ...
    print "1", Cache.get("lect","code")
    print "2", Cache.get("lect", "name")
    print "3", Cache.get("lect", "exam")


    self.ids["txt_exam_name"].text = Cache.get("lect",
                                               "code"
                                               )
    examId = find(find(Cache.get("info","courses"),"courseName",Cache.get("lect", "name"))["exams"],"name",Cache.get("lect", "exam"))
    ...
    if len(self.data_exam_order) < 1:
        ...
        print self.data_detailed_exam
        print "2"

        with open("data/questions.fay", "w+") as questions:
            i = 0
            q = ""
            for Q in self.data_detailed_exam:
                q += str(Q["id"]) + "*[SEAS-NEW-LINE]*" + \
                             str(Q["questionType"]) + "*[SEAS-NEW-LINE]*" + \
                             str(Q["value"]) + "*[SEAS-NEW-LINE]*" + \
                             str(Q["text"]) + "*[SEAS-NEW-LINE]*"
        ...
        self.question_no = "1"
        self.ids["txt_question_no"].text = "question no"
        self.question_type = "Type"
        self.question_grade = "Grade"
        self.ids["txt_question_grade"].text = "Q Grade"
        self.ids["txt_question_body"].text = "Q Body"
        print self.data_exam_order
