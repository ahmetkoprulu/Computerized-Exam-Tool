...
def find(source, purpose, name):
    for object in source:
        if object[purpose] == name:
            return object
...
def on_pre_enter(self):
    ...
    # self.data = ["asd","asdasd","asdasd"]
    self.DATA = Cache.get("info","courses")
    ...
    #                                             Cache.get("info", "nick"),Cache.get("info","id")
    #                                             )

    # for key in data_lectures:
    #     self.data.append("{i1}_{i0}".format(i1=key,i0 = data_lectures[key]))

    # for i in data_lectures["courses"]:
    ...
    for lect in self.DATA:
        btn_lect = Button(text=lect["courseName"],
                          color=(0, 0, 0, 1),
...
def on_lect_select(self, dropdown, txt):
    ...
    self.CourseInfo = find(self.DATA, "courseName", txt)

    self.ids["txt_lect_code"].text = str(self.CourseInfo["courseCode"])
    self.ids["txt_lect_name"].text = self.CourseInfo["courseName"]
...
def on_exams(self):
    ...
    self.AllData = Cache.get("info","courses")

    self.ExamDetails = database_api.getExamsOfLecture(Cache.get("info", "token"),find(self.AllData,"courseCode",self.ids["txt_lect_code"].text)["courseId"])
    print self.ExamDetails


    # self.data_exam_details = find(self.AllData,"courseCode", self.ids["txt_lect_code"].text)
    # print "Details"
    # print self.data_exam_details
    ...
    args_converter = lambda row_index, x: {"text": x,
                                           ...
                                           "background_down": "data/img/widget_gray_75.png",
                                           "background_normal": "data/img/widget_list_{x5}.png".format(x5="red"),
                                           "font_name": "data/font/CaviarDreams_Bold.ttf",
                                           "font_size": self.height / 25,
                                           "size_hint_y": None,
                                           "height": self.height / 10
                                           }
    self.ids["list_exams"].adapter = ListAdapter(data=[i["name"] for i in self.ExamDetails],
                                                 cls=ListItemButton,
    ...
    def on_filter(clr, dt):
        ...
        if clr != "all":
            ...
            print "2"
            self.ids["list_exams"].adapter.data = [exam["name"] for exam in self.ExamDetails]
    ...
    # Clock.schedule_interval(partial(self.on_exams),10)
...
def on_exam_select(self, dt):
    ...
    self.ExamSelected = find(self.ExamDetails,"name",self.ids["list_exams"].adapter.selection[0].text)

    self.ids["txt_status_body"].text = Status(self.ExamSelected["examStatusId"])
...
def on_exam_edit(self, dt):
    ...
    data_exam_questions = database_api.getExam(Cache.get("info", "token"),self.ExamSelected["examId"])
    print data_exam_questions
    ...
    args_converter = lambda row_index, x: {"text": "Question {:02}".format(int(x["num"])) + " ({type}...)".format(type=State(x["questionTypeId"])),
                                           "markup": True,
                                           "selected_color": (.843, .82, .82, 1),
                                           "deselected_color": (.57, .67, .68, 1),
                                           "background_down": "data/img/widget_gray_75.png",
                                           "background_normal": "data/img/widget_list_{t}.png".format(t=color(x["questionTypeId"])),
                                           "font_name": "data/font/CaviarDreams_Bold.ttf",
    ...
    self.list_edit.adapter = ListAdapter(data=sorted([i for i in data_exam_questions["questions"]]),
                                         cls=ListItemButton,
                                         args_converter=args_converter,
                                         allow_empty_selection=False
                                         )
