...
def on_exam_create(self):
    ...
    if self.ids["input_examname"].text.strip() and self.ids["input_duration"].text.strip():
        ...
        Cache.append("lect","date","{d} {t}".format(d=date, t=time))
        Cache.append("lect","duration",self.ids["input_duration"].text)

        CourseId = find(Cache.get("info","courses"),"courseName",self.ids["txt_lect_name"].text)["courseId"]


        ExamReq = database_api.createExam(CourseId,Cache.get("info", "token"),
                                self.ids["txt_lect_code"].text,
                                self.ids["input_examname"].text,
        ...
        print "After"
        if ExamReq.ok:
            Cache.append("CurrentExam","id",ExamReq.json()["id"])
            print Cache.get("CurrentExam","id")
            return self.on_question_add()
        else:
            print  ExamReq.status_code
            return 1
